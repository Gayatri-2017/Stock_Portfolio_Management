...
# df = pd.read_csv("../cluster_analysis/Results_with_sentiment/Profiles_qtrwise_cl4/1_quarter_profile.csv")
app = Flask(__name__)
...
# all_ohlcv_data = data_operations.get_all_ohlc_data_from_db(tablename="ohlc", KEYSPACE=KEYSPACE_NAME)
# all_ohlcv_data["time"] = pd.to_datetime(all_ohlcv_data['time'].dt.date)

# all_ohlcv_data = data_operations.get_all_ohlcv_data_from_db(tablename="ohlcv", KEYSPACE=KEYSPACE_NAME)
# all_ohlcv_data =  data_operations.get_all_ohlcv_data_from_csv(filename="/Users/amoghkallihal/Documents/CMPT-733/Project/Git\ Code/stock-portfolio-management/Code/OHLC_Features/ohlcv.csv").toPandas()
all_ohlcv_data = pd.read_csv("../OHLC_Features/ohlcv.csv")
all_ohlcv_data['time'] = pd.to_datetime(all_ohlcv_data['time'], format='%d/%m/%Y')
print('*'*80,all_ohlcv_data.head())
# pca_quarter_1 = data_operations.get_pca_quarter_1_data_from_db(tablename="pca_quarter_1", KEYSPACE=KEYSPACE_NAME)

# print("pca_quarter_1 = \n", pca_quarter_1)

# all_pca = data_operations.get_all_pca_data_from_db(tablename="all_pca", KEYSPACE=KEYSPACE_NAME)
# all_pca = get_all_pca_data_from_csv(filename="./pca_folder/pca_complete_result.csv").toPandas()

# print("all_pca = \n", all_pca)

def get_all_pca_data_from_csv(folder_name="../k_means_cluster_analysis/Results_with_sentiment/pca_folder_cl3/"):
    ...
    # pca_complete_result.csv
    # data.show()
    return complete_df

all_pca_kmeans_with_sentiment = get_all_pca_data_from_csv(folder_name="../k_means_cluster_analysis/Results_with_sentiment/pca_folder_cl3/")
all_pca_kmeans_without_sentiment = get_all_pca_data_from_csv(folder_name="../k_means_cluster_analysis/Results_without_sentiment/pca_folder_cl3/")

all_pca_kmedoids_with_sentiment = get_all_pca_data_from_csv(folder_name="../K_medoids_cluster_analysis/Results_with_sentiment/pca_folder_cl3/")
all_pca_kmedoids_without_sentiment = get_all_pca_data_from_csv(folder_name="../K_medoids_cluster_analysis/Results_without_sentiment/pca_folder_cl3/")

# pg_df = data_operations.get_portfolio_generation_two_year_with_sentiment_from_csv(folder_name="../data_operations/profile_generation/")
# pg_df = pd.read_csv("two-year-with-senti.csv")
# print("*"*80)
# print("pg_df = \n", pg_df)
# df = all_pca_with_sentiment
...
# quarter_list=["01/04/2018", "01/07/2018", "01/10/2018", "01/01/2019", "01/04/2019", "01/07/2019", "01/10/2019", "01/01/2020", "01/04/2020"]
quarter_list=["2018-04-01", "2018-07-01", "2018-10-01", "2019-01-01", "2019-04-01", "2019-07-01", "2019-10-01", "2020-01-01", "2020-04-01"]
# quarter_list = [pd.to_datetime(i, format='%d/%m/%Y') for i in quarter_list]
...
# profile_table_col_names = ['Parameters', 'Overall', 'KM_1', 'KM_2', 'KM_3']

clustering_method_list = ["K-means", "K-medoids"]
data_list = ["Linear Method using Min-Max", "Efficient Frontier Top 5 portfolios over 2 years", "Efficient Frontier Portfolio generated quarterwise"]
...
with app.app_context():
    ...
    # ], style={'background-image': 'url("./static/images/background_1.gif")'})
...
def render_content(tab):
    ...
    #     return html.Div([
            ...
            #     html.Div([
            #     html.Div([
            #         dcc.Graph(id='portfolio_generation_graph')
            #              ]
            #              , style={'display': 'inline-block', 'width': '47%', 'margin-left': '1.2%'}, className='six columns'),
            #          ], className='row',style={'padding': '10px 5px'})
...
def get_all_ohlc_time_median(df):
    # Remove tablename from declaration and call 
    df['time'] = pd.to_datetime(df['time'], format='%d/%m/%Y')
...
def ohlc_time_sector_median(xaxis_column_name, time_period):
    if(xaxis_column_name == "Select Sector"):
        ...
        if(time_period == 9):
            ...
            ohlc_time_sector_median_df = get_all_ohlc_time_median(ohlc_time_sector_median_df)
...
def ohlc_company(sector_name, company_name, time_period):
    ...
    ohlc_company_df['time'] = pd.to_datetime(ohlc_company_df['time'], format='%d/%m/%Y')
...
def get_company_lst(sector_name):
    ...
    top20companises_sectorwise = pd.read_csv("../top20companises_sectorwise.csv")
    print('*'*80,len(top20companises_sectorwise))
    company_list = top20companises_sectorwise[top20companises_sectorwise["Sector"] == sector_name]["company_symbol"].tolist()
    name_list = top20companises_sectorwise[top20companises_sectorwise["Sector"] == sector_name]["Name"].tolist()
    print('*'*80,company_list)
    print('*'*80,name_list)
    ...
    # return [company.__getitem__("company_symbol") + " - " +  name.__getitem__("Name") for company in company_list for name in name_list]
    # return [company_list[i].__getitem__("company_symbol") + " - " +  name_list[i].__getitem__("Name") for i in range(len(company_list))]
    return [company_list[i]+ " - " +  name_list[i] for i in range(len(company_list))]
...
def display_company_list(sector_name):
    company_list = get_company_lst(sector_name)
...
def line_graph(df, title, feature_name):
    ...
    # line_graph = px.line(df[90:], x='time', y=feature_name)
...
def ohlc_time_sector_median(xaxis_column_name, time_period, feature):
    ...
    if(xaxis_column_name == "Select Sector"):
        ...
        feature_ohlc_time_sector_median_df = get_all_ohlc_time_median(feature_ohlc_time_sector_median_df)
        feature_ohlc_time_sector_median_df = get_feature_df(df=feature_ohlc_time_sector_median_df, feature=feature_name)  
        if(time_period == 9):
...
def feature_company(company_name, time_period, feature):
    ...
    feature_name = feature.split()[0]
    print("feature_name = \n", feature_name)
    print("company_symbol = ", company_symbol)
    
    if(feature == "Select Feature"):
        title = "No Feature Selected"
    else:
        if(time_period == 9):
            ohlcv_feature_df = get_feature_df(all_ohlcv_data, company_symbol, feature_name)
...
def cluster_graph(time_period, senti_option, clustering_method):
    ...
    if(time_period != 9):
        ...
        print("for time_period = ", time_period, " df = \n", df)

    cluster0 = dict(
            x=df[df["label"]==0]['PC_0'],
            y=df[df["label"]==0]["PC_1"],
            # z=df[df["label"]==0]["pc_2"],
...
# The column names used are from Cassandra Database. 

# def cluster_graph(time_period, senti_option):
...
def cluster_company_graph(hoverData, feature, time_period):
    ...
    ohlcv_feature_df = get_feature_df(df, company_symbol, feature_name)
...
def generate_quarter_profile_table(time_period, senti_option):
    ...
    # print("sentiment_option = \n", senti_option)

    if(senti_option == sentiment_option[1]):
        print("Inside if")
        print("sentiment_option = \n", sentiment_option)
        df = get_profile_with_sentiment_from_csv(folder_name="../k_means_cluster_analysis/Results_with_sentiment/Profiles_qtrwise_cl3/", time_period=time_period)
    elif(senti_option == sentiment_option[2]):
        print("Inside elif")
        print("sentiment_option = \n", sentiment_option)
        df = get_profile_with_sentiment_from_csv(folder_name="../k_means_cluster_analysis/Results_without_sentiment/Profiles_qtrwise_cl3/", time_period=time_period)
...
# @dashapp.callback(
#     [Output("portfolio_generation_two_year_with_sentiment_table", "data")]
# )

# def generate_quarter_profile_table():
    
#     # sentiment_option = ["Select Sentiment Option", "With News Sentiment", "Without News Sentiment"]
#     print("*"*80)
#     df = get_portfolio_generation_two_year_with_sentiment_from_csv(folder_name="../data_operations/profile_generation/")
#     print("Inside generate_quarter_profile_table")
#     print("df = \n", df)
...
def image_content(clustering_method, data_method, sentiment_method):
    ...
    print("*"*80)
    print("Inside image_content")

    # root_path = "/Users/apple/Desktop/BD-2/Project/Our_gitlab_1/Code/"
    root_path = "./static/"
    if(sentiment_method == sentiment_list[0]):
        if(clustering_method == clustering_method_list[0]):
            # folder_name="./static/images/with_sentiment/k_means/"
        ...
        return (html.Div(
                    ...
                    html.Div([
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"min_max_ef.png",
                        ...
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"min_max_ind.png", style={'width': '98%', 'height': '600px'})
    ...
    # "Top 5 portfolios over 2 years"
       ...
       #  cols_list = ['Unnamed: 0', 'Sharpe Ratio', 'Portfolio Annualised Return',
       # 'Portfolio Annualised_Volatility', 'Stock Portfolio',
       # 'Stocks Weight Allocation', 'Portfolio Tag']
        ...
        return (html.Div(
                    ...
                    html.Div([
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_1_ef.png",
                        ...
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_1_ind.png", style={'width': '98%', 'height': '600px'})
                        ...
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_2_ef.png",
                        ...
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_2_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        ...
                        # html.Img(src=folder_name+"top5_3_ef.png",
                    ...
                    html.Div([
                        ...
                        # html.Img(src=folder_name+"top5_3_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_4_ef.png",
                        ...
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_4_ind.png", style={'width': '98%', 'height': '600px', "opacity": "100%"})
                    ...
                    html.Div([
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_5_ef.png",
                        ...
                        html.Div([
                            ...
                            # html.Img(src=folder_name+"top5_5_ind.png", style={'width': '98%', 'height': '600px'})
    ...
    # "Portfolio generated quarterwise"
       ...
       #  cols_list = ['Unnamed: 0', 'Quarter Start Date_x', 'Sharpe Ratio',
       # 'Portfolio Annualised Return', 'Portfolio Annualised_Volatility',
       # 'Stock Portfolio', 'Stocks Weight Allocation', 'Portfolio Tag']
        ...
        return (html.Div(
                    ...
                    html.Div([
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 1',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q1_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 1',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q1_ind.png", style={'width': '98%', 'height': '600px'})
                        ...
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 2',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q2_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 2',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q2_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.P(
                            'Plot of Efficient Frontier for Quarter 3 ',
                            className="control_label",
                            style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                   'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                   'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                        ),
                        # html.Img(src=folder_name+"q3_ef.png",
                    ...
                    html.Div([
                        html.P(
                            'Plot of Individual Stock Returns and Volatility for Quarter 3',
                            className="control_label",
                            style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                   'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                   'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                        ),
                        # html.Img(src=folder_name+"q3_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 4 ',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q4_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 4',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q4_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 5',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q5_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 5',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q5_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 6 ',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q6_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 6',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q6_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 7 ',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q7_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 7',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q7_ind.png", style={'width': '98%', 'height': '600px'})
                    ...
                    html.Div([
                        html.Div([
                            html.P(
                                'Plot of Efficient Frontier for Quarter 8 ',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q8_ef.png",
                        ...
                        html.Div([
                            html.P(
                                'Plot of Individual Stock Returns and Volatility for Quarter 8',
                                className="control_label",
                                style={'textAlign': 'center', 'width': '50%', 'margin-left': '25%',
                                       'padding': '10px 10px 10px 10px', 'borderBottom': 'thin lightgrey solid',
                                       'backgroundColor': 'rgb(250, 250, 250)', 'font-family': 'Helvetica', 'font-size': '18px'}
                            ),
                            # html.Img(src=folder_name+"q8_ind.png", style={'width': '98%', 'height': '600px'})
...
def generate_static_bar_charts(feature_name):
    ...
    print("*"*80)
    print("Inside generate_static_bar_charts")

    # if(feature_name==static_bar_chart_feature_list[0]):
...
def generate_static_line_charts(feature_name):
    ...
    print("*"*80)
    print("Inside generate_static_line_charts")
...
def generate_static_cluster_eda(sentiment_method, clustering_method):
    ...
    print("*"*80)
    print("Inside generate_static_cluster_eda")

    root_path = "../"
    print("sentiment_method = ", sentiment_method, " clustering_method = ", clustering_method)
    ...
    # pg_df = pd.read_csv("./static/csv/cluster_membership.csv")
